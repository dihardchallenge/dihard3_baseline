...
for f in $xvecdir/xvector.scp $xvecdir/spk2utt $xvecdir/utt2spk $xvecdir/segments $pldadir/$pldafilename $pldadir/mean.vec $pldadir/transform.mat; do
...
if [ $stage -le 0 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/plda_scoring.JOB.log \
    ivector-plda-scoring-dense --target-energy=$target_energy $pldadir/$pldafilename \
